...
class DocExampleScripts_Test(TestCase):
    ...
    def mocked_requests_get(*args, **kwargs):
        ...
        if args[0] == 'https://api.dataplatform.ibm.com/v2/analytics_engines/1234567890/state':
            return MockResponse({'state': 'Active'}, 200)
    ...
    def test(self, mock_get, mock_post):
        ...
        try:
            ...
            scriptfile = os.path.abspath(os.path.join(scriptDir, 'cluster_dp_api_status.py'))
            try:
                # Python 2x
                execfile(scriptfile)
            except:
